...
def analyze_performance(df_gcp, df_storm, duration_seconds):
    ...
    interval_sec = 30.0 # Realtime telemetry uses a 30-second window
    ...
    for idx, row in df_gcp.iterrows():
        ...
        # A. Node Energy Model: P_idle + (P_max - P_idle) * CPU_fraction
        profile = get_power_profile(vm_name)
        p_cpu = profile["p_idle"] + (profile["p_max"] - profile["p_idle"]) * cpu_usage_fraction
        
        # B. Link Energy Model: Bandwidth * Base_Energy * Latency * Link_Factor
        bytes_per_sec = bytes_sent / interval_sec
