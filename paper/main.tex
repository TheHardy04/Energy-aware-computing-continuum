...
\section{Energy aware continnum}
...
\end{table}
% --- END PROS/CONS TABLE ---
\section{State of the Art: Energy Formulation}
...
The modeling of server energy consumption is generally divided between dynamic models, which fluctuate with resource usage, and state-based models, which rely on active versus idle statuses.

\subsubsection{Utilization-Based Linear Models}
A common approach in resource scheduling is to model server energy as a linear function of CPU utilization. Jeong et al. propose a model for Federated Edge Clouds where the total energy of a server, $E^{server}$, consists of a static base load and a dynamic component proportional to the CPU load \cite{jeong2023towards}:
$$
    E^{server} = E^{static}_{server} + \left( E^{max}_{server} - E^{static}_{server} \right) \times \frac{CPU^{used}}{CPU^{total}}
$$
where $E^{static}_{server}$ represents the idle power consumption, and $E^{max}_{server}$ denotes the consumption at peak load. This approach is widely adopted for its simplicity and applicability to virtual machine (VM) scheduling. Similarly, Naha et al. utilize PlanetLab workload traces to derive energy consumption based on regression analysis of CPU utilization in Fog devices, emphasizing empirical validation over theoretical modeling \cite{naha2022multiple}.

\subsubsection{State-Based Integral Models}
...
\subsection{Physics-Based Component Models}
...
$$
where $c$ is a capacitance constant, $f$ is the frequency, and $A_m$ represents static memory power in the idle state. Furthermore, Adhikari et al. extend this model to quantify carbon footprints ($CE$) using emission factors $\gamma$:
$$
...
\subsection{Game-Theoretic Edge Server Management}
Cui et al. address the trade-off between minimizing active physical machines and maximizing user coverage through a game-theoretic formulation (EESM) \cite{cui2022energy}. Unlike centralized optimization, they model the problem as a \textit{Potential Game} where users autonomously select servers. To encourage workload consolidation, the decision benefit $B$ for a user joining server $j$ is proportional to the user density on that machine:
$$
    B = \frac{N_j}{e_j}
$$
where $N_j$ is the count of users currently served by server $j$, and $e_j$ is the server's total energy consumption (combining switching and running costs). 
...
$$
    Z = \begin{cases} 
    \frac{e_j}{N_j} & \text{Active User} \\
    \alpha \cdot e_0 & \text{Unserved User} 
    \end{cases}
$$
Here, $\alpha$ is a weighting parameter that prioritizes service coverage over raw energy savings, ensuring the system converges to a Nash Equilibrium that minimizes wasted energy while maintaining service levels.
