...
\bibliographystyle{ieeetr}% Conserver cette ligne pour la version finale
...
\title{Energy-Aware Computing Continuum}
\author{Théo HARDY}
\date{2025}
...
\ac{CC} has established itself as the ubiquitous backbone of modern computing environments, providing the scalable resources necessary for computation-intensive applications. However, this centralized model faces a critical sustainability limit driven by the explosive growth of the \ac{IoT}. With projections indicating tens of billions of connected devices by 2030, relying solely on remote cloud servers necessitates transporting massive volumes of data across the network. This reliance on centralized processing not only induces significant latency and congestion but also incurs a substantial, often overlooked, energy cost associated with the network infrastructure and data transport itself. It is estimated that by 2030, data centers alone could consume up to 8\% of the global electrical supply, a significant portion of which is still derived from non-renewable sources \cite{jones2018stop}.

To address these challenges, \acf{EC} and \acf{FC} have emerged as essential paradigms that complement the traditional cloud architecture. By migrating data processing tasks closer to the data sources—such as on-device processing or intermediate network gateways—these architectures significantly reduce latency and bandwidth consumption.

This evolution has led to the conceptualization of the \acf{CECC}. Rather than treating Cloud, Fog, and Edge as rigid, separate layers, the Continuum views them as a single, cohesive infrastructure. This unified approach allows for the dynamic placement of tasks across the infrastructure based on specific requirements, such as real-time processing needs or privacy constraints. Brings computing resources closer to resource-constrained IoT devices, thereby reducing bandwidth consumption and fostering an open innovation ecosystem that breaks the monopoly of proprietary cloud. Crucially, this architectural flexibility also presents a vital opportunity to reduce energy consumption: by optimizing service placement strategies, workloads can be intelligently distributed across the continuum to minimize the total power usage of digital services.

The remainder of this article is organized as follows. \Cref{sec:background} presents an overview of the foundational computing paradigms—Cloud, Fog, and Edge—and introduces the concept of the unified Cloud-Edge Computing Continuum. \Cref{sec:energy_formulation} provides a detailed literature review of mathematical energy formulations, categorized into server and cloud models, IoT-specific constraints, communication energy costs, and evaluation methods including simulators and testbeds. Finally, the report synthesizes these findings through comparative tables summarizing key optimization objectives and measurement methodologies found in recent research.

\section{Introduction on Edge/Fog Computing}\label{sec:background}
...
\section{Literature review: Devices Energy Formulation}\label{sec:energy_formulation}
...
\begin{tabularx}{\textheight}{ X X ccc cccc X X X X }
    ...
    2019 \cite{saraswat2019energy} &
	Data forwarding / Task splitting &
	\cmark & \cmark & \cmark &
	\cmark & \xmark & \cmark & \cmark &
	Primal barrier Newton method &
	State-based (Idle \& Active) &
	Periodic UbiComp (Classification) &
...
\subsubsection{Simulation Tools and Frameworks} Simulations provide a scalable, cost-effective alternative to physical measurements in complex, highly distributed environments. They allow researchers to model new ideas for system components (processors, memory, accelerators) or complete systems to estimate metrics such as performance and energy \cite{muralidhar2022energy}.

\paragraph{CloudSim and its Extensions \cite{calheiros2011cloudsim}} As the de-facto standard for cloud modeling, CloudSim has been extended through various projects to address specific modeling needs. \textbf{CloudSimSDN} \cite{son2015cloudsimsdn} extends the base framework to simulate Software-Defined Networking (SDN) features, allowing for the evaluation of network-aware VM placement policies and energy savings in SDN-enabled data centers. It was used by Jeong et al. \cite{jeong2023towards}. \textbf{EdgeCloudSim} \cite{sonmez2018edgecloudsim} extends CloudSim to address specific edge computing challenges such as mobility, dynamic networks, and load generation. It enables the evaluation of IoT applications and edge services by characterizing mobility and network models. A key component is the Edge Orchestrator, which handles resource management decisions. EdgeCloudSim is extensible, configurable, and generates CSV logs for analysis. However, it currently supports only partial scalability (tier-based clustering) and lacks comprehensive energy models for the entire continuum, though future extensions aim to address energy, task failures, and VM migration. \textbf{iFogSim} \cite{gupta2017ifogsim} is specifically designed to model Fog and Edge architectures, enabling researchers to evaluate resource management policies. Recent enhancements have significantly expanded its capabilities: Baneshi et al. integrated flow-based networking energy models (using flow-based and time-based approaches for NICs) alongside computation energy to provide end-to-end sustainability estimates \cite{baneshi2024analyzing}. Furthermore, \textbf{iFogSim2} \cite{mahmud2022ifogsim2} addresses the dynamic nature of the continuum by introducing modular components for mobility management (handling service migration as devices move), dynamic node clustering (enabling distributed resource sharing), and microservice orchestration. Unlike the original version, iFogSim2 supports the direct integration of real-world datasets (e.g., EUA dataset) and optimizes simulation performance, reducing memory usage and execution time by approximately 28\%.

\paragraph{Specialized and Hybrid Simulators} Beyond general-purpose cloud simulators, specialized tools address specific layers of the continuum. NS-3 is widely utilized for detailed, low-level device networking simulations, offering high fidelity for communication protocols but often lacking high-level application modeling \cite{baneshi2024analyzing}. In contrast, SimGrid offers highly accurate modeling for distributed systems, including virtual machines and server energy consumption, though it typically lacks native components for detailed network energy modeling in Fog scenarios \cite{ahvar2019estimating}.

\paragraph{AI-Integrated Environments} Modern frameworks support the training of intelligent schedulers. COSCO (Container Orchestration using Co-simulation) \cite{tuli2021cosco} allows for the development and testing of AI-based scheduling policies (e.g., Reinforcement Learning) in integrated Edge, Fog, and Cloud environments \cite{iftikhar2023hunterplus}. Similarly, interfaces like OpenAI Gym \cite{brockman2016openai} are now being used to train advanced schedulers (such as CNN or RL models) to adapt to non-stationary settings where workload characteristics change erratically \cite{goudarzi2021distributed}.

\subsubsection{Experimental Testbeds}

\paragraph{Grid'5000 \cite{cappello2005grid}} 
French large-scale experimental testbed that is a primary resource for obtaining high-precision energy profiles of compute nodes. Researchers use it to conduct stress benchmarks on heterogeneous hardware, recording power consumption per loaded core with external wattmeters accurate to 0.125 Watts \cite{ahvar2019estimating}. Grid'5000 is internationally recognized and serves as a foundation for new scale platforms such as FutureGrid in the USA \cite{balouek2012adding}.

\paragraph{Edge-Specifig Clusers}
Many studies utilize custom-built testbeds consisting of heterogeneous hardware like Raspberry Pis and Jetson Nanos to mirror real-world Edge deployments \cite{laso2025energy,goudarzi2021distributed,saraswat2019energy}. 

\paragraph{Public Cloud Infrastructure} Virtual machines from providers such as Azure (Standard B2s) are also used to test schedulers in a hybrid Cloud-Fog context \cite{iftikhar2023hunterplus}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{ 
        ...
        % --- DATA ROWS (Sorted Chronologically 2025 -> 2019) ---
