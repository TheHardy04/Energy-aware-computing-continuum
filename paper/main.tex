...
While previous models focus on total transmission duration, Baneshi et al. argue that in the computing continuum (Edge-Fog-Cloud), energy must be attributed to specific applications sharing the infrastructure \cite{baneshi2024analyzing}. They propose a flow-based model for shared devices (e.g., routers and gateways) that allocates the active energy of Network Interface Cards (NICs) based on the bandwidth proportion used by an application:
