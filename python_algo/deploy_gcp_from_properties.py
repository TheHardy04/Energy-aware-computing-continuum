...
def vm_exists(vm_name):
    ...
    cmd = f'gcloud compute instances describe {vm_name} --zone={ZONE} --format="value(name)" --quiet'
...
def get_vm_ip(vm_name):
    ...
    import json
    ip_cmd = f'gcloud compute instances describe {vm_name} --zone={ZONE} --format=json'
...
def ensure_storm_firewall_rule():
    ...
    check_cmd = f'gcloud compute firewall-rules describe {FIREWALL_RULE} --format="value(name)" --quiet'
    ...
    create_cmd = (
        ...
        f"--network={NETWORK} "
        f"--allow=tcp:6700-6799,tcp:6627,tcp:2181,tcp:8080 "
        f"--source-tags=storm-node "
        f"--target-tags=storm-node"
    )
...
def main():
    ...
    if vm_exists(MASTER_NAME):
        ...
        cmd_master = (
            f"gcloud compute instances create {MASTER_NAME} "
