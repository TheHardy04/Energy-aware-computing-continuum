...
%----------------------------------------------------------------------------------------
%
{ \bf  \huge {Energy Aware Computing Continuum} \\  \Large \it Energy Modeling Across the Cloud-Edge Computing Continuum, Comparative Synthesis and practical guideline}
...
\begin{itemize}
\item The \textbf{Cloud-Edge Computing Continuum (CECC)} enables flexible task placement but introduces a \textbf{complex optimization landscape}. Orchestrating resources necessitates balancing strict performance constraints (latency, data locality) with the minimization of operational costs and energy consumption.
\vspace{-0.2cm}
\item To address the fragmentation of existing metrics, this research establishes a \textbf{comprehensive taxonomy and comparative synthesis} of operational energy formulations. We analyze models based on device scope, abstraction level, and measurement methodology to identify gaps in current service placement strategies.
\vspace{-0.2cm}
\item Furthermore, we bridge the gap between theory and practice by deploying a \textbf{real-world experiment on Google Cloud Platform (GCP)}. We benchmark the energy accuracy of distinct resolution paradigms—including using a multi-component Data Stream Processing (DSP) application.
\end{itemize}
}
