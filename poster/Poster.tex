...
% --- GCP Storm Topology Architecture ---
%\colorbox{benchmarkcol!12}
{\parbox{0.95\linewidth}{\centering\footnotesize%
\textbf{GCP Architecture — Apache Storm Deployment}\\[4pt]
\resizebox{0.92\linewidth}{!}{%
\begin{tikzpicture}[
  every node/.style={font=\sffamily\small},
  vmbox/.style={draw=neutral, rounded corners=3pt, thick, fill=neutrallight, minimum width=3.8cm, minimum height=2.2cm, align=center},
  schedbox/.style={draw=tiercloud, rounded corners=3pt, thick, fill=tiercloud!10, minimum width=3.2cm, minimum height=0.9cm, align=center},
  supbox/.style={draw=benchmarkcol, rounded corners=2pt, thick, fill=benchmarkcol!12, minimum width=2.8cm, minimum height=0.55cm, align=center},
  taskbox/.style={draw=esilvcol, rounded corners=2pt, thick, fill=esilvcol!8, minimum width=1.1cm, minimum height=0.45cm, align=center, font=\sffamily\scriptsize},
  nimbusbox/.style={draw=tiercloud, rounded corners=2pt, thick, fill=tiercloud!18, minimum width=2.4cm, minimum height=0.55cm, align=center, font=\sffamily\small\bfseries},
  arrstyle/.style={-{Stealth[length=5pt]}, thick, neutral!60},
]
% Scheduler VM
\node[vmbox, minimum width=3.8cm, minimum height=1.8cm] (sched) at (0, 3.0) {};
\node[font=\sffamily\small\bfseries, anchor=north] at (0, 3.85) {Scheduler GCP VM};
\node[nimbusbox] (nimbus) at (0, 3.0) {Nimbus};

\node[anchor=north] at (5, 3.85) {\includegraphics[height=1.5cm]{gcp.png}};
\node[anchor=north] at (7, 3.85) {\includegraphics[height=1.5cm]{apache.png}};

% Worker VM S1
\node[vmbox] (w1) at (-3.5, 0.9) {};
\node[supbox] (sup1) at (-3.5, 1.4) {Supervisor 1};
\node[font=\sffamily\small\bfseries, anchor=south] at (-3.5, -0.15) {Worker GCP VM S1};
\node[taskbox] (t1a) at (-4.2, 0.5) {Task};
\node[taskbox] (t1b) at (-2.8, 0.5) {Task};

% Worker VM S2
\node[vmbox] (w2) at (3.5, 0.9) {};
\node[supbox] (sup2) at (3.5, 1.4) {Supervisor 2};
\node[font=\sffamily\small\bfseries, anchor=south] at (3.5, -0.15) {Worker GCP VM S2};
\node[taskbox] (t2a) at (2.8, 0.5) {Task};
\node[taskbox] (t2b) at (4.2, 0.5) {Task};

% Supervisor N (dots)
\node[font=\sffamily\bfseries, text=neutral] at (6.2, 1.2) {\large $\cdots$};
\node[supbox, minimum width=1.6cm, fill=benchmarkcol!6] at (7.6, 1.2) {\scriptsize Supervisor N};

% Arrows
\draw[arrstyle] (nimbus.south) -- ++(0,-0.3) -| (sup1.north);
\draw[arrstyle] (nimbus.south) -- ++(0,-0.3) -| (sup2.north);
\draw[arrstyle] (sup1.south) -- ++(0,-0.15) -| (t1a.north);
\draw[arrstyle] (sup1.south) -- ++(0,-0.15) -| (t1b.north);
\draw[arrstyle] (sup2.south) -- ++(0,-0.15) -| (t2a.north);
\draw[arrstyle] (sup2.south) -- ++(0,-0.15) -| (t2b.north);
\end{tikzpicture}
}%
}}
\begin{itemize}
    \item Nimbus schedules topology components to Workers based on pre-computed placement (Greedy-E/CSP/LLM)
    \vspace{-2mm}
    \item VMs energy monitoring validates simulation
\end{itemize}
}}
...
%\vspace{0.1cm}
\begin{itemize}[leftmargin=*, itemsep=-2pt]
  \item Scaling study on \textbf{larger instances}.
