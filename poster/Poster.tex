...
\definecolor{headercol1}{RGB}{206,16,82}      % Header left (ESILV)
\definecolor{headercol2}{RGB}{206,16,82}      % Header right (ESILV)
\definecolor{headerfontcol}{RGB}{255,255,255}% Header text (white)
\definecolor{boxcolor}{RGB}{250,250,248}      % Content (off-white)
\definecolor{taxonomycol}{RGB}{34,139,34}     % Taxonomy - Green
\definecolor{benchmarkcol}{RGB}{204,85,0}     % Real-World Benchmarking - Orange
...
\toprule
\textbf{Metric} & \textbf{Greedy (FFD)} & \textbf{CSP (OPTIMAL)} & \textbf{LLM (gemini-2.0)} \\
\midrule
...
\textbf{Contributions:} Unified energy taxonomy and practical guidelines for task orchestration on GCP. Mini-survey on energy aspects across heterogeneous infrastructure. Paper for COMPAS conference documenting energy modeling and real-world benchmarking.
...
%	ACKNOWLEDGEMENTS
%----------------------------------------------------------------------------------------
\headerbox{Acknowledgements}{name=acknowledgements,column=3,below=results, ,span=3}{
\smaller
We thank supervisors \textbf{Farah Ait Salaht} for microservice optimization insights, and \textbf{WLADDIMIRO Daniel} for enabling real-world GCP and Apache Storm deployment. Work conducted at ESILV.
}
